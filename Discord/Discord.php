<?php

require_once __DIR__.'/Exceptions/DiscordEmbedLimitException.php';
require_once __DIR__.'/Exceptions/DiscordInvalidResponseException.php';
require_once __DIR__.'/Exceptions/DiscordOAuthCodeExpiredException.php';

require_once __DIR__.'/Models/DiscordObjectParser.php';

require_once __DIR__.'/Models/DiscordGuildMember.php';
require_once __DIR__.'/Models/DiscordGuildPreview.php';
require_once __DIR__.'/Models/DiscordGuildStageInstance.php';
require_once __DIR__.'/Models/DiscordGuildWelcomeScreen.php';
require_once __DIR__.'/Models/DiscordGuildWelcomeScreenChannel.php';

require_once __DIR__.'/Models/DiscordSticker.php';
require_once __DIR__.'/Models/DiscordStickerItem.php';

require_once __DIR__.'/Models/DiscordGuildWidget.php';
require_once __DIR__.'/Models/DiscordConnection.php';
require_once __DIR__.'/Models/DiscordIntegration.php';
require_once __DIR__.'/Models/IntegrationApplication.php';
require_once __DIR__.'/Models/IntegrationAccount.php';

require_once __DIR__.'/Models/ThreadMember.php';
require_once __DIR__.'/Models/ThreadMetadata.php';
require_once __DIR__.'/Models/VoiceRegion.php';

require_once __DIR__.'/Models/DiscordTeamMember.php';
require_once __DIR__.'/Models/DiscordTeam.php';
require_once __DIR__.'/Models/DiscordApplication.php';

require_once __DIR__.'/Models/DiscordInviteStageInstance.php';
require_once __DIR__.'/Models/DiscordInviteMetadata.php';
require_once __DIR__.'/Models/DiscordInvite.php';

require_once __DIR__.'/Models/DiscordChannelOverwriteObject.php';

require_once __DIR__.'/Models/BanObject.php';
require_once __DIR__.'/Models/DiscordRoleTags.php';
require_once __DIR__.'/Models/DiscordRole.php';
require_once __DIR__.'/Models/DiscordEmoji.php';
require_once __DIR__.'/Models/DiscordUser.php';
require_once __DIR__.'/Models/DiscordGuild.php';
require_once __DIR__.'/Models/DiscordChannel.php';
require_once __DIR__.'/Models/DiscordEmbed.php';
require_once __DIR__.'/Models/DiscordMessage.php';

require_once __DIR__.'/DiscordBot.php';
require_once __DIR__.'/DiscordRequest.php';
require_once __DIR__.'/DiscordOAuth.php';
require_once __DIR__.'/DiscordWebhook.php';